...
internal open class RenderNodeApi21(name: String?) : RenderNodeWrapper {
    ...
    private fun recordEmptyDisplayList() {
        val canvas = RenderNodeApi21Reflector.start(renderNode, 0, 0)
        RenderNodeApi21Reflector.end(renderNode, canvas)
    }
    ...
    override fun beginRecording(): Canvas =
        RenderNodeApi21Reflector.start(renderNode, width, height)
            .also { canvas = it }
    ...
    override fun setUseCompositingLayer(
        ...
        renderNode.setLayerType(if (forceToLayer) 2 else 0) or
                renderNode.setLayerPaint(paint)

    override fun setHasOverlappingRendering(hasOverlappingRendering: Boolean) =
        renderNode.setHasOverlappingRendering(hasOverlappingRendering)
}

private object RenderNodeApi21Reflector {

    private val startMethod =
        RenderNode::class.java
            .getDeclaredMethod("start", Int::class.java, Int::class.java)

    private val endMethod =
        RenderNode::class.java
            .getDeclaredMethod("end", HardwareCanvas::class.java)
